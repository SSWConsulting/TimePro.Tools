...
/// <summary>
/// Builds the markdown body for the skills (no frontmatter, no run-these block).
/// This is the single source of truth for the long shared prose.
/// </summary>
public static class SkillBodyBuilder
{
    /// <summary>
    /// The deterministic, read-only commands rendered in the portable
    /// "run these first" block.
    /// </summary>
    ...
    /// <summary>
    /// Builds the shared timesheets skill body.
    /// </summary>
        ...
        string? ghRepoSlug)
    {
        ...
        sb.AppendLine("# TimePro Timesheets");
        sb.AppendLine();
        sb.AppendLine("## Setup");
        ...
        sb.AppendLine("clients that have no repo mapping yet. Use it to choose the project before");
        sb.AppendLine("filling timesheets — it is the natural \"where do I start?\" step.");
        sb.AppendLine();
        sb.AppendLine("If more than one project is plausible, ask the user which one to use before");
        sb.AppendLine("continuing.");
        sb.AppendLine();
        ...
        sb.AppendLine("1. **Pick the project**: `tp project recent --json` to choose from likely candidates.");
        sb.AppendLine("2. **Pre-flight**: verify repo mapping is configured: `tp map detect`");
        sb.AppendLine("   If category is missing, run the \"Repo Mapping Setup\" workflow below first.");
        sb.AppendLine("3. Check existing: `tp ts get --week --json`");
        sb.AppendLine("4. Check CRM bookings: `tp bk list --week --json`");
        sb.AppendLine("5. **Decide per day: accept suggestions OR create new entries (never both)**");
        ...
        sb.AppendLine("6. Gather context for each day:");
        ...
        sb.AppendLine("7. Accept or create timesheets with GH issue/PR references in the description");
        sb.AppendLine("8. Verify: `tp ts get --week` to confirm all days are covered");
        sb.AppendLine();
    ...
    /// <summary>
    /// Builds the shared accounting skill body (no frontmatter). The accounting
    /// flows are exploratory, not deterministic, so this skill carries no
    /// prefetch commands.
    /// </summary>
    public static string BuildAccountingBody(TenantConfig? tenant)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("# Client billable-work threshold report");
        sb.AppendLine("tp client billable-work --from 2025-06-26 --to 2026-06-26 --threshold 50000 --json");
        sb.AppendLine("tp client billable-work --from 2025-06-26 --to 2026-06-26 --threshold 50000 --output ./Reports/client-billable-work.csv");
        sb.AppendLine();
        ...
        sb.AppendLine("```");
        sb.AppendLine();
        sb.AppendLine("### 6. Clients above a billable-work threshold");
        sb.AppendLine();
        sb.AppendLine("```bash");
        sb.AppendLine("tp client billable-work --from 2025-06-26 --to 2026-06-26 --threshold 50000 --json \\");
        ...
        sb.AppendLine("work in the selected window is at or above the threshold. Use `--json` for");
        sb.AppendLine("agent workflows or `--output ./Reports/<file>.csv` when a CSV artifact is");
        sb.AppendLine("needed.");
        sb.AppendLine();
        sb.AppendLine("### 7. Credit-note audit");
        sb.AppendLine();
        sb.AppendLine("```bash");
