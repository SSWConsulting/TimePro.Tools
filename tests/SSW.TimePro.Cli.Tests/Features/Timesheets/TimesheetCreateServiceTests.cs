...
public class TimesheetCreateServiceTests
{
    ...
    [Fact]
    public async Task Prepare_WhenTheDateIsMalformed_ThrowsAValidationError()
    {
        var service = new TimesheetCreateService(ApiWithRate(), Config());

        var thrown = await Assert.ThrowsAsync<TimesheetValidationException>(
            () => service.PrepareAsync(Emp, Options() with { Date = "16/03/2026" }, Ct));

        thrown.Message.Should().Contain("Use yyyy-MM-dd");
    }
